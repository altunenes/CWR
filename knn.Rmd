...
But what about KNN which stands for K nearest algoritm? Remember that, while K-means is using for "clustering" problems KNN is using for the classification problems. *K-means* clustering **is an unsupervised** clustering approach, whereas *KNN* is a **supervised learning algorithm** used for **classification**.
...
According to this explanation, we could select k as 1 or 2. If we select k=2; our accuracy will be %96 with %94 kappa value. According to Cohen, Kappa \> .9 indicates that model is almost perfect than expected by chance.
